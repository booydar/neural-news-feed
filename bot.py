...
class NewsBot(AsyncTeleBot):
    ...
    def load_messages(self):
        ...
        rated_ids = {str(r['id']) + '-' + str(r['channel_id']) for r in self.ratings}
        ...
        print(f"Found {len(self.messages)} total messages")
    ...
    def load_ratings(self):
        ...
        if os.path.exists(RATINGS_PATH):
            ...
            print(f"Found {len(self.ratings)} ratings")
        else:
            print("Ratings file not found. Starting with no ratings.")
    ...
    def get_new_messages(self, n=3):
        new_messages = self.messages[:n]
        self.messages = self.messages[n:]
        return new_messages
    
    def rate(self, message, rating, is_advertisement=False):
        print(message)
        ...
        if hasattr(self, "channel_ratings"):
            message['ranking_method'] = "channel_ratings_v3"
...
def rate_markup(msg_id, channel_id):
    markup = InlineKeyboardMarkup()
    num_rates = 5
    markup.row_width = num_rates + 1
    buttons = [InlineKeyboardButton("Ad", callback_data=f"rate_{msg_id}_{channel_id}_ad")]
    buttons += [InlineKeyboardButton(str(r), callback_data=f"rate_{msg_id}_{channel_id}_{r}") for r in range(1, num_rates+1)]
    markup.add(*buttons)
    return markup
    ...
    try:
        for message in messages:
            async with TelegramClient(username, api_id, api_hash) as client:
                msg_id, channel_id = int(message['id']), int(message['channel_id'])
                await client.forward_messages(chat_id, msg_id, channel_id)
            msg_out = await bot.send_message(bot.chat_id, "Rate this post", reply_markup=rate_markup(msg_id, channel_id))
            bot.current_messages[(msg_id, channel_id)] = (message, msg_out)
...
async def callback_query(call):    
    if call.data.startswith("rate_"):
        msg_id, chat_id, rating = call.data.split("_")[1:]
        # print(f"Rating: {msg_id, chat_id, rating }")
        # print(bot.current_messages)
        message, msg_out = bot.current_messages.pop((int(msg_id), int(chat_id)))
        await bot.delete_message(bot.chat_id, msg_out.message_id)
        if rating == 'ad':
            bot.rate(message, 0, True)
            await bot.delete_message(bot.chat_id, msg_out.message_id - 1)
        else:
            bot.rate(message, int(rating), False)
            if int(rating) < 4:
                await bot.delete_message(bot.chat_id, msg_out.message_id - 1)
    ...
    print(len(bot.current_messages))
    if len(bot.current_messages) == 0:
        await main()
